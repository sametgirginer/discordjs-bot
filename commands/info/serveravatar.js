...
module.exports = {
    ...
    run: async (client, message, args) => {
		if ((!message.mentions.users.size && !args.length) || message.type === "REPLY") {
			...
			if (!member.avatar) return infoMsg(message, 'Red', await buildText("serveravatar_notfound", client, { guild: message.guild.id, message: message }), true, 5000);
			...
			if (!user) return infoMsg(message, 'Red', await buildText("user_notfound", client, { guild: message.guild.id, message: message }), true, 5000);
			if (!member.avatar) return infoMsg(message, 'Red', await buildText("serveravatar_notfound", client, { guild: message.guild.id, message: message }), true, 5000);
