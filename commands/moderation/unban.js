...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (!args[0]) return infoMsg(message, 'Red', await buildText("unban_id_required", client, { guild: message.guild.id }), true, 5000); 

        let bannedMember;
        try {
            bannedMember = await client.users.fetch(args[0]);
        } catch (error) {
            if(!bannedMember) return infoMsg(message, 'Red', await buildText("unban_id_incorrect", client, { guild: message.guild.id }), true, 5000); 
        }
    
        try {
            await message.guild.bans.fetch(args[0]);
        } catch(error) {
            return infoMsg(message, 'Red', await buildText("unban_user_not_banned", client, { guild: message.guild.id }), true, 5000); 
        }
